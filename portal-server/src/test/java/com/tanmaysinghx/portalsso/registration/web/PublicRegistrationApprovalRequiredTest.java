...
/** With no email verification yet, approval mode is what keeps a human in the loop. */
@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
@TestPropertySource(properties = {"app.registration.enabled=true", "app.registration.require-admin-approval=true"})
class PublicRegistrationApprovalRequiredTest {
    ...
    @Test
    void theAccountIsCreatedDisabledSoItCannotSignInYet() throws Exception {
        ...
        assertThat(userRepository.findByEmail(email).orElseThrow().isEnabled())
                .as("must wait for an administrator")
                .isFalse();
    }
}
